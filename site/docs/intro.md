...
title: はじめに
slug: /
description: Mac で動いている Codex を iPhone から再開するための入口です。
---
...
Mac 側の準備がまだなら、先に <MacDownloadLink>CodexPocketMac の入手方法</MacDownloadLink> を確認しておくと、そのまま Pairing まで進めます。

<MacDownloadLink className="button button--primary button--lg">Mac 側の準備を始める</MacDownloadLink>
...
初回設定は、Mac と iPhone が同じローカルネットワークにいる状態で行います。設定後に Managed Relay 情報が保存された Host は、別 Wi-Fi やモバイル回線からも接続できます。細かな環境整備やトラブル対応は、必要に応じて Mac 側で行います。
...
- 画像、Fork、ブランチ切り替えなど、Mac に戻るほどではない操作を iPhone から済ませたい
