...
- `ホーム`
  既定 Host、最近使ったプロジェクト、お気に入りを見返す入口です。
- `プロジェクト`
...
2. 初回起動で権限と表示設定を確認する
3. `ホスト` で Mac の QR を読み取り、Host を保存する
...
> TODO(画像): `ホーム / プロジェクト / ホスト / 設定` の 4 タブが見える iPhone 全体のスクリーンショット。初めて開いた人が、どこから何をするアプリかを一目で把握できる画像があると分かりやすくなります。
