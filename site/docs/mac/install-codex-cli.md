...
- ターミナルでは動くのに Mac アプリでは認識されないときは、Mac アプリが login shell の `which -a codex` と標準的な standalone / Homebrew / Codex app の場所も確認します。それでも見えない場合は、`詳細` の `実行ファイルパス` で `Codex CLI` に `which codex` の結果または実行ファイルのパスを指定します
