...
5. 必要に応じて `Fork`、`ブランチ`、`スキル` を使う

## 追加指示の送り方

- `キュー`
  Composer 右側の `キュー` から送ります。今の実行が終わってから次の依頼を送ります
- `ステア`
  Composer 左側の `ステア` から送ります。進行中の実行にそのまま追加指示を送ります

`キュー` した内容は保留カードとして残り、そこから `すぐステア` か `キャンセル` を選べます。
...
- [Branch / Skills / Fork を使う](../usage/branch-skills-fork)
