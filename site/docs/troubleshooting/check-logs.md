...
`実行ファイルパス` では、`Codex CLI`、`git`、`シェル` を個別に指定できます。空に保存すると、その項目だけ自動検出へ戻ります。
...
`詳細` の `Codex CLI` を見ます。Mac アプリは app から見える PATH だけでなく、login shell の `which -a codex` と標準的な standalone / Homebrew / Codex app の場所も確認します。それでも見つからない場合は、`実行ファイルパス` で `Codex CLI` に `which codex` の結果または実行ファイルのパスを指定します。

### `git missing`

通常の Thread 操作だけなら大きな問題にならないことがあります。ただし、ブランチ切り替えや作成を使いたいなら、`git` が見えている必要があります。Terminal では使えるのに Mac アプリで見つからない場合は、`実行ファイルパス` で `git` の実行ファイルを指定します。
