...
`詳細` の `Codex CLI` を見ます。`which codex` が通っても、Mac アプリから別の PATH を見ていることがあります。Mac アプリは標準の Homebrew / system install path も確認するため、再起動しても変わらない場合はインストール先と PATH を見直します。
