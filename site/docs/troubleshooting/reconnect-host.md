...
> TODO(画像): iPhone の `ホスト` 画面で `接続テスト` と `設定を再試行` の位置が分かる画面。再 Pairing の前に何を試すべきかを伝えやすくなります。
> TODO(画像): Mac の `一般` と `Bridge` を並べた画面。`ランタイム`、`ヘルス`、ポート、endpoint の関係を説明しやすくなります。
