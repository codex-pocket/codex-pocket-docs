...
description: This page explains how to open, read, and continue a thread.
---
...
1. Open a `Project`.
2. Tap `New Thread` or choose an existing `Thread` from the list.
3. Once the conversation loads, read through the latest content.
...
- a spinning indicator
  Work is still in progress in that `Thread`.
- a blue dot
  There are unread updates.
- one-line preview
  Lets you skim the latest content.
- updated time
  Helps you tell which `Thread` is newest.
...
- `Work Log`
  The part that shows what Codex is doing along the way.
- `Answer`
  The user-facing response as a finished reply.
- `Back to latest`
  Jumps you to the end when new output arrives while you are reading older content.
...
- keep the same `Thread` when it is the same piece of work
- use `New Thread` when it is a different task
- use `Fork` from `...` when you want to try a different direction while keeping the current context
